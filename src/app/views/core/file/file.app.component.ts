...
import { FileAppService, File, Folder, Metric } from './service/file.app.service';
...
export class FileAppComponent implements OnInit {
    ...
    getMetricPercentage(metric: Metric): string {
        if (metric.current > metric.total) return '100%';
        return ((metric.current / metric.total) * 100).toFixed(0) + '%';
    }
