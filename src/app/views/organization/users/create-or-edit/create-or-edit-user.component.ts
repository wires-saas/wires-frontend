...
export class CreateOrEditUserComponent implements OnInit {
    ...
    get organizationName$(): Observable<string> {
        return this.organizationService.currentOrganization$.pipe(
            map((org) => org?.name || ''),
        );
    }
    ...
    async ngOnInit() {
        const userFoundInUrl: string =
            this.activatedRoute.snapshot.params['user'];
    ...
    private async fetchUserAndInitForms() {
        ...
        if (userFoundInUrl) {
            this.userBeingModified =
                await this.userService.getUserById(userFoundInUrl);
    ...
    async onSubmit() {
        if (this.isInvitingNewUser) {
            ...
            if (this.canSaveAvatar() && this.nextAvatar) {
                await this.userService
                    .uploadAvatar(this.userBeingModified._id, this.nextAvatar)
                    .then(() => {
            ...
            if (this.canSaveProfile()) {
                await this.userService
                    .updateUser(this.userBeingModified._id, this.userForm.value)
                    .then(() => {
                        this.userSavedState = this.userForm.value;
