...
export class FeedFrequencyPipe implements PipeTransform {
    transform(feed: Feed, type: 'auto' | 'manual' = 'manual'): string {
        ...
        const granularity = type === 'auto' ? feed.autoScrapingGranularity : feed.scrapingGranularity;
        const interval = type === 'auto' ? feed.autoScrapingInterval : feed.scrapingInterval;
